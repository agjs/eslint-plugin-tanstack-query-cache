...
function getSpreadPrefixText(node, getText) {
  ...
  if (firstElement === null || firstElement === void 0 || firstElement.type !== AST_NODE_TYPES.SpreadElement) {
    return null;
  }
...
function findQueryKeyProperty(obj) {
  return obj.properties.find(
    (p) => p.type === AST_NODE_TYPES.Property && !p.computed && p.key.type === AST_NODE_TYPES.Identifier && p.key.name === "queryKey"
  );
}
...
function collectExtendedPrefixes(program, getText) {
  ...
  walkAst(program, (node) => {
    ...
    const { callee } = node;
    if (callee.type !== AST_NODE_TYPES.Identifier || !HOOK_NAMES.has(callee.name) || node.arguments[0]?.type !== AST_NODE_TYPES.ObjectExpression) {
      return;
    }
...
function queryFilterAllowsPrefixMatch(arg) {
  if (arg === void 0 || arg.type !== AST_NODE_TYPES.ObjectExpression) {
    return false;
  }
  for (const prop of arg.properties) {
    if (prop.type !== AST_NODE_TYPES.Property || prop.computed || prop.key.type !== AST_NODE_TYPES.Identifier) {
      continue;
    }
    if (prop.key.name === "predicate") {
      return true;
    }
    if (prop.key.name === "exact" && prop.value.type === AST_NODE_TYPES.Literal && prop.value.value === false) {
      return true;
    }
  }
  return false;
